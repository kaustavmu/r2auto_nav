...
class AutoNav(Node):
    ...
    def launcher_callback(self, msg):
        ...
        if (msg.data == 'target hit'):
            ...
            self.stopbot()
            if self.timeystart == 0.0:
                self.timeyend = 0.0
            else:
                self.timeyend = time.monotonic()
           ...
           #dist_target = self.laser_range[100]         # at ~70cm, degree of ir ~100
            ...
            twist.angular.z = rotatechange
            self.publisher_.publish(twist)
            if not self.launcher_ready:
                self.timeystart = time.monotonic()
            self.launcher_ready = True
            self.turner = 1
    ...
    def spin_once_shoot(self):
        if (self.found_target):
            ...
            diff = self.timeyend - self.timeystart
            self.get_logger().info('timeystart: %f' % self.timeystart)
            self.get_logger().info('timeyend: %f' % self.timeyend)
            self.get_logger().info('diff: %f' % diff)
            aang = rotatechange*self.turner*diff*0.8
            self.timeystart = 0.0
            self.timeyend = 0.0
            #self.get_logger().info('final yaw: %f' % self.final_yaw)
            
            self.on_target = False
            self.found_target = False
            #self.manual_yaw = self.yaw
            #diff = self.final_yaw - self.init_yaw
            self.get_logger().info('aang radians: %f' % aang)
            self.get_logger().info('aang degrees: %f' % math.degrees(aang))
            if self.fired_once == True:
                self.manual_yaw += aang 
            self.pick_direction()
            return
        else:
            rclpy.spin_once(self)
    ...
    def pick_direction(self):
        ...
        if self.grid_x == -1 or self.grid_y == -1:
            print('I feel like Im Gucci Mane 2006')
        ...
        elif self.grid_x != -1 and self.grid_y != -1:
            print('All these diamonds on my neck cost me like 4 bricks')
